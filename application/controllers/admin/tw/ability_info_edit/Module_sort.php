...
{
  // 基本設定
  public $table = 'tw_ability_info_edit'; // 資料庫table name // 須修改
  ...
  ***********/
  ...
  public function read($id = NULL)
  {
    ...
    // 排序:置入某筆資料
    if($id) $data['data'] = $this->admin_crud->result_array($this->admin_crud->query_where($this->table, array('pid !='=>$id, 'ofkpid'=>$this->session->userdata('ofkpid')), true, 'xsort', 'asc'));
    else {
      $data['data'] = $this->admin_crud->result_array($this->admin_crud->query_where($this->table, array('ofkpid'=>$this->session->userdata('ofkpid')), true, 'xsort', 'asc'));
      $actionIndex = true;
    }
    // 處理列表中圖片
      ...
      $this->input->post('xtitle', true)
        ...
        if(!isset($data['error'])) {
          ...
          $data = array(
            ...
            'ofkpid'=> $this->session->userdata('ofkpid'),
