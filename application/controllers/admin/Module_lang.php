...
// controllers 名稱 // 須修改
class Module_lang extends Admin_Controller
{
  // 基本設定
  public $table = 'admin_lang'; // 資料庫table name // 須修改
  ...
  public function save($id = NULL)
  {
    if(
      ...
      $this->input->post('xcode', true)
        ...
        $this->db->where('xcode',$xcode);
        if($id) $this->db->where_not_in('pid',$id);
    ...
    if($file) {
      ...
      switch ($type) {
        ...
        case 'csv':
          ...
          if($this->common->ImportCsv($file,$this->table,$array,$compareIndex))
            $msg .= '成功';
          else $msg = '未匯入任何資料';
        break;
        ...
        case 'xlsx':
          ...
          $compareIndex = 'xtitle'; // 從 array 欄位做為比對依據
          if($this->common->ImportExcel($file,$this->table,$array,$compareIndex))
            $msg .= '成功';
          else $msg = '未匯入任何資料';
        break;
        default:
