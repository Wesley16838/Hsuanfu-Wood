...
// controllers 名稱 // 須修改
class Module_multisort_caty extends Admin_Controller
{
  // 基本設定
  public $MultiLevel = 1; // 類別層級(最少1層)(值=2，2層類別+1層詳細頁) // 須修改
  public $table = 'multisort_caty'; // 資料庫table name // 須修改
  protected $jointable = 'multisort_relative'; // 關連資料庫 // 須修改
  // 頁面設定
  public $subindexPath = 'admin/module_multisort_app'; // 須修改

  /***********
    列表頁
  ***********/
  // 表格-呈現欄位 // 須修改
  public $tableTitle = array('刊登','主標','小圖'/*,'其他功能'*/,'切換');
  public $tableColumn = array('xpublish','xname','xfile1'/*,''*/,'next');
  ...
  public function save($id = NULL)
  {
    if(
      $this->input->post('xname', true)
        ...
        if($xname && !$xurltitle) {$xurltitle = $this->common->emptyAuto($xname,'true');
          if($msg = $this->common->processUrl($this->table,$id,'xurltitle',$xurltitle)) $data['error'] = $msg;
        }
        if($xname && !$xseotitle) $xseotitle = $this->common->emptyAuto($xname,'false');
  ...
  public function sort()
  {
    ...
    $where['preid'] = $this->session->userdata('preid');
    $this->admin_crud->sorting($this->table,$obj[0],$start,$where,'pid');
  }
