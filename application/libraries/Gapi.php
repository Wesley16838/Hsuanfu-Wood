  ...
  public function fetchToken($client_email, $key_file, $delegate_email = null) {
    ...
    if (!file_exists($key_file)) {
      if ( !file_exists(__DIR__ . DIRECTORY_SEPARATOR . $key_file) ) {
        ...
        throw new Exception('GAPI: Failed load key file. File could not be found.');
      } else {
    ...
    if (empty($key_data)) {
      ...
      throw new Exception('GAPI: Failed load key file. File could not be opened or is empty.');
    }
    ...
    if (!isset($certs['pkey'])) {
      ...
      throw new Exception('GAPI: Failed load key file. Unable to load pkcs12 check if correct p12 format.');
    }
