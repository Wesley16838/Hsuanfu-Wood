                      ...
                      <div id="tab-1" class="tab-pane active"> <!--須修改-->
                        <div class="panel-body">
                          <fieldset class="form-horizontal">
                            ...
                            <div class="form-group"><label class="col-sm-2 control-label">語系/代號:</label>
                              <div class="col-sm-10"><input type="text" class="form-control" name="xcode" value="<?=(isset($list['xcode']))?$list['xcode']:''?>" placeholder="" required></div>
                            </div>
          ...
          render: function ( data, type, row ) { // 顯示資料值前，先處理資料值
            switch (xcolumn) {  // 需修改 // 每個欄位都需設定
              case 'xtitle':
                ...
                break;
              case 'xpublish':
                if(data.xpublish == 'yes') var checked = 'checked';
                else var checked = '';
                return '<input type="radio" '+checked+' value="'+data.xpublish+'">';
                break;
              default:
                return data;
                break;
...
<script>
  function forminit() {
    ...
    var action = '<?=$action?>';
    if(action=='import') {
      var msg = '<?=(isset($uploadmsg))?$uploadmsg:""?>';
      if(msg) {
        alert(msg);
        redirect("<?=site_url($this->indexPath)?>");
      }
    }
  }
</script>
