<?php $this->subnavPath = $this->common->getsubnav($this->session->preid,$this->table); // 固定 ?>
<!--麵包屑-->
...
<?php if((int)($this->session->userdata('nowlevel') < $this->MultiLevel)):?>
<!--內容-->
...
<?php if($action == 'index'):?>
          ...
          <div class="user-ctrl-nav padding"> <!--須修改-->
            ...
            <?=generatebutton('link_self_href', 'btn btn-default', 'javascript:AutoUpdSeo("'.$this->table.'","xname,xname,xsubtitle","xurltitle,xseotitle,xseodescription","true,false,false")', $permission['CreateAction'], '同步SEO');?>
                  ...
                  <div class="tabs-container">
                      ...
                      <div class="tab-content">
                          <div id="tab-1" class="tab-pane active"> <!--須修改-->
                              <div class="panel-body">
                                ...
                                <fieldset class="form-horizontal">
                                  ...
                                  <?=$this->load->view('admin/module/form',array('name'=>'主標','em'=>'xname','type'=>'title','autoArray'=>array(
                                    array('autoname'=>'Rewrite','xtb'=>$this->table,'inputName'=>'xname','outputName'=>"xurltitle",'outputType'=>'input','isurl'=>true),
                                    array('autoname'=>'Seo title','xtb'=>$this->table,'inputName'=>'xname','outputName'=>"xseotitle",'outputType'=>'input','isurl'=>false)
                                  )),true)?>
                          ...
                          <div id="tab-3" class="tab-pane"> <!--須修改-->
                            <div class="panel-body">
                              <?=$this->load->view('admin/module/form',array('name'=>'URL Rewrite','em'=>'xurltitle','type'=>'url','inputfield'=>'xname','xtb'=>$this->table),true)?>
                              <?=$this->load->view('admin/module/form',array('type'=>'seo','showitem'=>'t','inputfield'=>'xname','xtb'=>$this->table),true)?>
...
<?php else: redirect($this->subindexPath);?>
<?php endif;?>
        ...
        options.columnDefs.push({
          ...
          render: function ( data, type, row ) { // 顯示資料值前，先處理資料值
            switch (xcolumn) {  // 需修改 // 每個欄位都需設定
              ...
              case 'next':
                var btn='';
                btn = '<button class="btn btn-default btn-xs m-b" onclick="redirect(\'<?=site_url($this->indexPath.'/index')?>'+'/'+data.pid+'\')">進入列表</button>';
                return btn;
              default:
                return data;
                break;
...
<script>
  function forminit() {
    $(function() {
      ...
      $("#createform").validate({
        rules: {
          xname:{required: true},xurltitle:{strings: true},
        },
        messages: {
          xname:{required: "欄位必填"},
        },
