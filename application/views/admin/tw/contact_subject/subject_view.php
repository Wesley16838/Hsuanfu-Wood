                          ...
                          <div id="tab-1" class="tab-pane active"> <!--須修改-->
                              <div class="panel-body">
                                <fieldset class="form-horizontal">
                                  ...
                                  <div class="form-group"><label class="col-sm-2 control-label">主旨:</label>
                                    <div class="col-sm-10"><input type="text" class="form-control" name="xsubject" value="<?=(isset($list['xsubject']))?$list['xsubject']:''?>" placeholder="" required></div>
                                  </div>

                                  <div class="form-group"><label class="col-sm-2 control-label">郵件標題:</label>
                                    <div class="col-sm-10"><input type="text" class="form-control" name="xmailsubject" value="<?=(isset($list['xmailsubject']))?$list['xmailsubject']:''?>" placeholder="" required></div>
                                  </div>

                                  <div class="form-group"><label class="col-sm-2 control-label">E-mail:</label>
                                    <div class="col-sm-10">
                                      <?php if (!isset($list['xmail']) || count($list['xmail'])==0): ?>
                                        <section class="copysection">
                                          <div class="col-sm-8">
                                            <input class='form-control' name="xmail[]" value="">
                                          </div>
                                          <div class="col-sm-2">
                                            <button class="btn btn-sm btn-primary add_new">+</button>
                                            <button class="btn btn-sm btn-danger remove_one">-</button>
                                          </div>
                                        </section>
                                      <?php else: ?>
                                        <?php foreach ($list['xmail'] as $value): ?>
                                          <section class="copysection">
                                            <div class="col-sm-8">
                                              <input class='form-control' name="xmail[]" value="<?=$value['xmail']?>">
                                            </div>
                                            <div class="col-sm-2">
                                              <button class="btn btn-sm btn-primary add_new">+</button>
                                              <button class="btn btn-sm btn-danger remove_one">-</button>
                                  ...
                                  <div class="form-group">
                                    ...
                                    <div class="col-sm-10">
                                      <div class="btn-group" data-toggle="buttons">
                                        ...
                                        <label><input type="radio" value="insert" id="insert" name="xsort" onchange="readSort('<?=(isset($list['pid']))?$list['pid']:''?>','xsubject')">置入所選資料之後</label>
        ...
        options.columnDefs.push({
          ...
          render: function ( data, type, row ) { // 顯示資料值前，先處理資料值
            switch (xcolumn) {  // 需修改 // 每個欄位都需設定
              case 'xsubject':
                return '<a href="<?=$this->formPath?>/'+data.pid+'">'+data.xsubject+'</a>';
                break;
              case 'xpublish':
